  ...
  onRead(data, uuid) {
    ...
    if (uuid == 'DEVICE_INFORMATION') {
      ...
      */
      ...
      // デバッグ: 受信したパケットのヘッダとサイズをログ出力
      console.log(`[SENSOR_VALUES] Header: ${data.getUint8(0)}, Length: ${data.byteLength} bytes`);
      ...
      if (data.getUint8(0) == 50) {
        ...
        // エラー処理 - 92バイトまたは104バイトに対応
        if (data.byteLength != 92 && data.byteLength != 104) {
          console.warn(`[SENSOR_VALUES] Header 50 packet length mismatch: expected 92 or 104 bytes, got ${data.byteLength} bytes. Packet will be ignored.`);
          return
        }
        
        // 104バイト版の場合、追加データの情報をログ出力（デバッグ用）
        if (data.byteLength == 104) {
          console.log(`[SENSOR_VALUES] Using 104-byte format (extended version)`);
        }
      ...
      // 未知のヘッダの場合
        ...
        console.warn(`[SENSOR_VALUES] Unknown header: ${data.getUint8(0)} (expected 40 or 50). Packet length: ${data.byteLength} bytes.`);
        // デバッグ用：最初の20バイトを16進数で表示
        let hexDump = '';
        for (let i = 0; i < Math.min(20, data.byteLength); i++) {
          hexDump += data.getUint8(i).toString(16).padStart(2, '0') + ' ';
        }
        console.log(`[SENSOR_VALUES] First 20 bytes (hex): ${hexDump}`);
        
        // フォールバック: パケット長が17バイト以上なら、ヘッダ40の形式として解析を試みる
        if (data.byteLength >= 17) {
          console.log(`[SENSOR_VALUES] Attempting to parse as header 40 format (fallback mode)`);
          try {
            this.quat = {
              w: data.getInt16(1) / 32768,
              x: data.getInt16(3) / 32768,
            ...
            // ジャイロと加速度補正をかけたものを別途作成
            let gyroRange = this.device_information?.range?.gyro || 250;
            let accRange = this.device_information?.range?.acc || 2;
            ...
            console.log(`[SENSOR_VALUES] Fallback parsing successful. Acc: (${this.acc.x.toFixed(2)}, ${this.acc.y.toFixed(2)}, ${this.acc.z.toFixed(2)})`);
          } catch (error) {
            console.error(`[SENSOR_VALUES] Fallback parsing failed:`, error);
          }
        } else {
          console.error(`[SENSOR_VALUES] Packet too short (${data.byteLength} bytes) for fallback parsing. Minimum 17 bytes required.`);
        }
      }
    }
  }
